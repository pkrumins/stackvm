...
function VM (opts) {
    ...
    rfb.addListener('startRects', function (rfbVars) {
        if (rfbVars.nRects > 1) {
            rfbVars.pngStack = new PngLib.DynamicPngStack();
        }
    });
    rfb.addListener('endRects', function (rfbVars) {
        if (rfbVars.nRects > 1) {
    ...
    rfb.addListener('raw', function (rect) {
        if (rect.nRects == 1) {
            ...
            rfbVars.pngStack.push(rect.fb, rect.x, rect.y, rect.width, rect.height);
        }
    });
