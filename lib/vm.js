...
function VM (opts) {
    ...
    this.attach = function (client) {
        ...
        DNode.connect(9200, function (screenshotRemote) {
            DNode.connect(9300, function (screencastRemote) {
                ...
                vm.addListener('startScreencast', function (fb, width, height) {
                    screencastRemote.startScreencast(width, height, function (videoId) {
                        ...
                        screencastRemote.newFrame(
                            videoId, fb.toString('binary'), (new Date).getTime(),
                            function () {}
                        );
                    });
                });
                ...
                vm.addListener('screencastPushUpdate', function (fb, x, y, w, h) {
                    screencastRemote.pushUpdate(
                        screencastId, fb.toString('binary'), x, y, w, h,
                        function (){}
                    );
                });
                vm.addListener('screencastEndPush', function () {
                    screencastRemote.endPush(
                        screencastId, (new Date).getTime(),
                        function (){}
                    );
                });
            });
