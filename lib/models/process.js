...
function Process (params) {
    ...
    self.saveThumb = function (cb) {
        var encoder = framebuffer.encoder;
        encoder.on('screenUpdate', function f (update) {
            if (update.fullScreen) {
                var dir = userdir + '/thumbs/' + self.filename;
                ...
                function makeImage () {
                    encoder.removeListener('screenUpdate', f);
                    fs.createWriteStream(dir + '/' + self.address + '.png')
                        .write(update.base64, 'base64');
                    cb(null, self.address + '.png');
                }
            }
        });
    };
