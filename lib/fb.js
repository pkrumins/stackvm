...
function FB (params) {
    ...
    self.screenshot = function (cb) {
        encoder.onceListener('raw', function g (rect) {
            rfb.fbDims(function (dims) {
                ...
                if (!fullScreen) {
                    ...
                    var image = Image('png', 'bgr').encode(rect.fb, rect.width, rect.height);
    ...
    DNode.connect(config.screencasts.dnodePort, function (remote) {
        var fileName = null;

        var pushFunc = function (rect) {
            remote.pushUpdate(fileName, rect.fb.toString('binary'),
                rect.x, rect.y, rect.width, rect.height);
        };

        var endPushFunc = function () {
            remote.endPush(fileName, (new Date).getTime());
        };

        self.startScreencast = function () {
            console.log('start screencast');
            rfb.fbDims(function (dims) {
                remote.startScreencast(dims.width, dims.height,
                    function (fileName_) {
                        fileName = fileName_;
                        encoder.on('raw', pushFunc);
                        encoder.on('endRects', endPushFunc);
                    }
                );
                rfb.requestRedrawScreen();
            });
        ...
        self.stopScreencast = function (cb) {
            ...
            remote.stopScreencast(fileName, function (status) {
                cb(config.screencasts.baseURI + '/screencasts/' + fileName);
                encoder.removeListener('raw', pushFunc);
                encoder.removeListener('endRects', endPushFunc);
            });
            return self;
        };
    });
